        ...
        <table cellspacing="1" cellpadding="2" width="100%" border="0" class="pn-ftable">
            ...
            <tr>
                ...
                <td width="80%" class="pn-fcontent">
                    ...
                    <input type="checkbox" value="9" name="colorss"
                           <c:forEach items="${product.colorss }" var="color">
                           <c:if test="${color==9 }">checked="checked"</c:if>
                    </c:forEach>/>西瓜红
                    <input type="checkbox" value="1" name="colorss"
                           <c:forEach items="${product.colorss }" var="color">
                           <c:if test="${color==1 }">checked="checked"</c:if>
                    </c:forEach>/>红色系
                    <input type="checkbox" value="2" name="colorss"
                           <c:forEach items="${product.colorss }" var="color">
                           <c:if test="${color==2 }">checked="checked"</c:if>
                    </c:forEach>/>绿色系
                    <input type="checkbox" value="3" name="colorss"
                           <c:forEach items="${product.colorss }" var="color">
                           <c:if test="${color==3 }">checked="checked"</c:if>
                    </c:forEach>/>蓝色系
                    <input type="checkbox" value="4" name="colorss"
                           <c:forEach items="${product.colorss }" var="color">
                           <c:if test="${color==4 }">checked="checked"</c:if>
                    </c:forEach>/>黑色系
                    <input type="checkbox" value="22" name="colorss"
                           <c:forEach items="${product.colorss }" var="color">
                           <c:if test="${color==22 }">checked="checked"</c:if>
                    </c:forEach>/>宝石蓝绿
                    <input type="checkbox" value="26" name="colorss"
                           <c:forEach items="${product.colorss }" var="color">
                           <c:if test="${color==26 }">checked="checked"</c:if>
                    </c:forEach>/>花色系
                </td>
            </tr>
            <tr>
                ...
                <td width="80%" class="pn-fcontent">
                    <input type="checkbox" value="S" name="sizess"
                           <c:forEach items="${product.sizess }" var="size">
                           <c:if test="${size=='S' }">checked="checked"</c:if>
                    </c:forEach>/>S
                    <input type="checkbox" value="M" name="sizess"
                           <c:forEach items="${product.sizess }" var="size">
                           <c:if test="${size=='M' }">checked="checked"</c:if>
                    </c:forEach>/>M
                    <input type="checkbox" value="L" name="sizess"
                           <c:forEach items="${product.sizess }" var="size">
                           <c:if test="${size=='L' }">checked="checked"</c:if>
                    </c:forEach>/>L
                    <input type="checkbox" value="XL" name="sizess"
                           <c:forEach items="${product.sizess }" var="size">
                           <c:if test="${size=='XL' }">checked="checked"</c:if>
                    </c:forEach>/>XL
                    <input type="checkbox" value="XXL" name="sizess"
                           <c:forEach items="${product.sizess }" var="size">
                           <c:if test="${size=='XXL' }">checked="checked"</c:if>
                    </c:forEach>/>XXL
                </td>
            </tr>
            <tr>
                ...
                <td width="80%" class="pn-fcontent">
                    <input type="checkbox" name="isNew" value="1"
                           <c:if test="${product.isNew==1 }">checked="checked"</c:if>/>新品
                    <input type="checkbox" name="isCommend" value="1"
                           <c:if test="${product.isCommend==1 }">checked="checked"</c:if>/>推荐
                    <input type="checkbox" name="isHot" value="1"
                           <c:if test="${product.isHot==1 }">checked="checked"</c:if>/>热卖
                </td>
            </tr>
            </tbody>
            ...
            <tr>
                <td>
                    <textarea rows="20" cols="180" id="productList" name="packageList"
                              value="${product.packageList }"></textarea>
                </td>
            </tr>
            </tbody>
            <tbody>
