...
function serachKeyword(){
	window.location.href = "productList?keyword=" + $("#keyword").val();
}
//点击 品牌   id == 品牌ID
function fqBrand(id){
	//判断是否有价格
	if(price != ''){
		window.location.href = "productList?keyword=" + keyword + "&brandId=" + id + "&priceStr=" + price;
	}else{
		window.location.href = "productList?keyword=" + keyword + "&brandId=" + id;
	}
}
//点击价格 id == 价格区间
function fqPrice(id){
	if(brandId != ''){
		window.location.href = "productList?keyword=" + keyword + "&brandId=" + brandId + "&priceStr=" + id;
	}else{
		window.location.href = "productList?keyword=" + keyword + "&priceStr=" + id;
	}
}
		...
		<li data-sku="1711416562" class="gl-item">
			<div class="gl-i-wrap">
				...
				<div class="p-name p-name-type-2">
					<a target="_blank" title="满129立减10,199减20优惠券,支持货到付款" href="javascript:;" onclick="window.open('http://localhost:8083/html/product/${product.id}.html')"">
						<em>${product.name }</em>
					</a>
				</div>
				<div class="p-commit">
